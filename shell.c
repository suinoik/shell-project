...
int main() {
    ...
    while (true) {
        ...
        char* tokenizer = strtok(command_line, delimiters);
        int i = 0;
        while (tokenizer != NULL) {
            tokenizer[strcspn(tokenizer, "\r\n")] = 0;
            arguments[i] = tokenizer;
      ...
      if (strcmp(arguments[0], "cd")==0) {
        ...
        if (pid < 0) {
          ...
          if (SIGINT) {
            signal(SIGINT, kill(pid, SIGKILL));
            printf("\n");
          }
          signal(SIGALRM, kill(pid, SIGKILL));
          alarm(10);
          wait(NULL);
        }
